...
- [ ] T014 [US3] **STOPPED 2026-09-23:** probe `Up` still has `AlterColumn` `Id` IdentityColumn on actor, collection, director, genre, movie, and player, plus `AddColumn` `Status`. SC-003 is not met. Probe discarded. See `research.md` Phase B pickup evidence. Run [quickstart.md](quickstart.md) §5. **Reopened under owner option A (2026-09-23):** the criterion below replaces the exactly-one-operation check. Record the pending-changes result and a probe `Up` containing `AddColumn` `Status`, plus at most the six baseline-residual `AlterColumn` `Id` IdentityColumn operations (actor, collection, director, genre, movie, player), each listed. Discard the probe so that `LamuFlix.Data/Migrations` has no diff (Patron H1). Do not edit the snapshot; DEV-19 owns the refresh. If the probe shows any other operation, stop and report to Patron with the T007 baseline; add no migration (Patron SF-1). Depends on T007, T013, and T013a.
