...
## 5. Pending-model check (FR-004, SC-003; AC4 as corrected by Patron H1 and amended by owner option A)

The model maps `Movie.Status` (`Movie.cs:21`; `LamuFlixContext.cs:54`), but no migration or snapshot contains it. So after DEV-360 the only pending operation DEV-360 may leave is the pre-existing `AddColumn` for `Status` on the movie table. The 2.1.3-era snapshot also yields the six pre-existing `AlterColumn` operations on `Id` that add `MySql:ValueGenerationStrategy` IdentityColumn to actor, collection, director, genre, movie, and player (T007 baseline). These are baseline residuals, not DEV-360 drift. DEV-19's fresh Initial migration refreshes the snapshot. No migration is committed and the snapshot is not edited.
...
3. List the pending operations with a throwaway probe: `dotnet tool run dotnet-ef migrations add Dev360PendingProbe --project LamuFlix.Data --startup-project LamuFlix.Data --context LamuFlixContext`. Record the exit (0). The generated `Up` must contain `AddColumn` named `Status` on the movie table. That column is non-nullable and keeps its `Pending` default (`LamuFlixContext.cs:54`). Apart from that, `Up` may contain only the six baseline-residual `AlterColumn` operations on `Id` (IdentityColumn on actor, collection, director, genre, movie, and player), and the record must list them. Any other operation fails SC-003, for example an `AlterColumn` left by nullability drift or an `UpdateData`.
