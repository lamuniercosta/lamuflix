...
- **Do not reset seats after ordinary asks.** Seats are reset only by `team-restart`, between tasks.
- **Recruit only with the seat map's launch command:** every `maestri recruit` carries `--command "<launch>"` taken from the seat's active rung in `seat-map.json`; verify with `maestri check "<Seat>"`.
- **A timed-out ask is not done:** run `maestri check "<Seat>"` first. If the seat is idle with no report, re-ask once; if it is still working, wait. Never mark the step complete on a timeout.
